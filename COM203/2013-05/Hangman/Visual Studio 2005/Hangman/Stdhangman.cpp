...
void cursorpos(int X, int Y)
{
	...
	SetConsoleCursorPosition( hStdOut, newpos ); // set the new curser position
}

// take the input modulous the length by 80, set the cursor position there
void centerline(string input, int linenum)
{
	if (input.length() > 80) // over 80 characters? must be centered already
	{
		cursorpos(0,linenum);
	}
...
// do the seperator thing
void drawseperator()
{
	for (int i = 2; i < 24; i++) // sepearates the hangman from the words
	{
		cursorpos(50,i);
		cout << "|";
	}
