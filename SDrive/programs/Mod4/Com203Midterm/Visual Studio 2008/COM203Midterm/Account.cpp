...
Account::Account(string namein)
{
	...
	unsigned int power = (unsigned int)pow((float)2,31); // pow(2,31) does not compile on visual studio 2008
	setaccountNumber(rand()%power);
	setaccountBalance((float)(rand()%power));
}
/******************************************************************************
